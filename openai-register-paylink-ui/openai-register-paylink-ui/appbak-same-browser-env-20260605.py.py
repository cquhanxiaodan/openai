...
PAYMENT_PROFILE_DIR = APP_DIR / "payment-browser-profiles"
...
def fingerprint_to_dict(fp: DeviceFingerprint) -> dict:
    return dataclasses.asdict(fp)


def fingerprint_from_dict(value: dict) -> DeviceFingerprint:
    return DeviceFingerprint(
        user_agent=str(value.get("user_agent") or DEFAULT_USER_AGENT),
        locale=str(value.get("locale") or "en-US"),
        languages=list(value.get("languages") or ["en-US", "en"]),
        timezone=str(value.get("timezone") or "America/New_York"),
        viewport_width=int(value.get("viewport_width") or 1365),
        viewport_height=int(value.get("viewport_height") or 768),
        screen_width=int(value.get("screen_width") or 1366),
        screen_height=int(value.get("screen_height") or 768),
        outer_width=int(value.get("outer_width") or 1376),
        outer_height=int(value.get("outer_height") or 860),
        device_scale_factor=float(value.get("device_scale_factor") or 1),
        hardware_concurrency=int(value.get("hardware_concurrency") or 8),
        device_memory=int(value.get("device_memory") or 8),
        platform=str(value.get("platform") or "Win32"),
        vendor=str(value.get("vendor") or "Google Inc."),
        max_touch_points=int(value.get("max_touch_points") or 0),
    )
...
def opll_new_http_session() -> requests.Session:
    if CurlCffiSession is not None:
        return CurlCffiSession(impersonate="chrome136")  # type: ignore[return-value]
...
def opll_create_checkout(access_token: str, country: str, currency: str, proxy_url: str = "") -> dict:
    ...
    return {"cs_id": str(cs_id), "processor_entity": processor_entity, "billing_country": country, "currency": currency}


def opll_stripe_init(cs_id: str, country: str, currency: str, proxy_url: str = "", payment_locale: str = "en", stripe: requests.Session | None = None, ctx: dict | None = None) -> dict:
...
def opll_stripe_create_paypal_method(stripe: requests.Session, cs_id: str, ctx: dict, billing: dict) -> str:
    body = {
        "billing_details[name]": billing.get("name") or "Taro Yamada",
        "billing_details[email]": billing.get("email") or "buyer@example.com",
        "billing_details[address][country]": billing.get("country") or "JP",
        "billing_details[address][line1]": billing.get("line1") or "1-1 Marunouchi",
        "billing_details[address][city]": billing.get("city") or "Tokyo",
        "billing_details[address][postal_code]": billing.get("postal_code") or "100-0005",
        "billing_details[address][state]": billing.get("state") or "Tokyo",
        "type": "paypal",
        "payment_user_agent": "stripe.js/5c62bd78d8; stripe-js-v3/5c62bd78d8; payment-element; deferred-intent",
...
def opll_extract_redirect_to_url(payload) -> str:
    ...
    if isinstance(next_action, dict) and next_action.get("type") == "redirect_to_url":
        ...
        if isinstance(redirect_to_url, dict) and redirect_to_url.get("url"):
            return str(redirect_to_url.get("url") or "").strip()
    for key in ("setup_intent", "payment_intent"):
        found = opll_extract_redirect_to_url(payload.get(key))
        if found:
            return found
    return ""
...
def opll_stripe_payment_page_redirect_url(stripe: requests.Session, cs_id: str, payment_locale: str = "en", timeout_seconds: int = 45, ctx: dict | None = None) -> str:
    ...
    while time.time() < deadline:
        ...
        if response.status_code == 200:
            redirect_url = opll_extract_redirect_to_url(response.json() or {})
            if redirect_url:
                return redirect_url
            last_err = response.text[:120]
        else:
...
def opll_resolve_external_redirect(stripe: requests.Session, redirect_url: str, preferred_host: str = "paypal.com") -> str:
    current = str(redirect_url or "").strip()
    for _ in range(5):
        host = (urlsplit(current).netloc or "").lower()
        if preferred_host and (host == preferred_host or host.endswith(f".{preferred_host}")):
            return current
...
def generate_opll_paypal_long_link(access_token: str, country: str, currency: str, proxy_url: str = "") -> dict:
    checkout = opll_create_checkout(access_token, "US", "USD", proxy_url)
    stripe = opll_build_stripe_session(proxy_url)
    ctx = opll_stripe_context({})
    init_payload = opll_stripe_init(checkout["cs_id"], checkout["billing_country"], checkout["currency"], proxy_url, stripe=stripe, ctx=ctx)
    ...
    provider_error = ""
    try:
        pm_id = opll_stripe_create_paypal_method(stripe, checkout["cs_id"], ctx, opll_random_jp_billing())
        return_url = opll_stripe_confirm_return_url(checkout["cs_id"], checkout, stripe_hosted_url)
        ...
        if confirm.status_code >= 400:
            raise RuntimeError(f"stripe confirm failed: HTTP {confirm.status_code} {confirm.text[:500]}")
        confirm_payload = confirm.json() or {}
        ...
        if not stripe_redirect_url and isinstance(submission, dict) and submission.get("state") == "requires_approval":
            opll_chatgpt_approve(opll_build_chatgpt_session(access_token, proxy_url), checkout["cs_id"], checkout)
            stripe_redirect_url = opll_stripe_payment_page_redirect_url(stripe, checkout["cs_id"], ctx=ctx)
        if not stripe_redirect_url:
            stripe_redirect_url = opll_stripe_payment_page_redirect_url(stripe, checkout["cs_id"], timeout_seconds=30, ctx=ctx)
        provider_url = opll_resolve_external_redirect(stripe, stripe_redirect_url)
        provider = {"payment_method_id": pm_id, "stripe_redirect_url": stripe_redirect_url, "provider_redirect_url": provider_url, "long_url": provider_url or stripe_redirect_url}
    except Exception as exc:
        fallback = True
        provider_error = str(exc)
    return {**checkout, "payment_method_id": provider["payment_method_id"], "stripe_hosted_url": stripe_hosted_url, "stripe_redirect_url": provider["stripe_redirect_url"], "provider_redirect_url": provider["provider_redirect_url"], "fallback": fallback, "provider_error": provider_error, "long_url": provider["long_url"] or hosted_long_url}
...
def generate_opll_hosted_long_link(access_token: str, country: str, currency: str, proxy_url: str = "") -> dict:
    ...
    init_payload = opll_stripe_init(checkout["cs_id"], checkout["billing_country"], checkout["currency"], proxy_url)
    stripe_hosted_url = str(init_payload.get("stripe_hosted_url") or "").strip()
    if not stripe_hosted_url:
        raise RuntimeError(f"stripe init response missing stripe_hosted_url, keys={sorted(init_payload.keys())}")
...
class OpenAIRegisterPayLinkWorker:
    def __init__(self, account: MailAccount, payment_mode: str, headless: bool, register_proxy: ProxyConfig, extract_proxy: ProxyConfig, log, registration_flow: str = "email", phone_provider=None):
        self.account = account
    ...
    def run(self) -> dict:
        with sync_playwright() as p:
            ...
            try:
                ...
                self._log_browser_proxy_status(register_page, self.register_proxy, "注册浏览器代理", require_japan=True)
    ...
    def _log_browser_proxy_status(self, page, proxy: ProxyConfig, label: str, require_japan: bool = False) -> None:
        if not proxy.chain_url:
            if require_japan:
                raise RuntimeError(f"{label}: 直连，注册要求日本代理出口")
            self.log(f"{label}: 直连")
            return
        try:
            ...
            summary = " ".join(part for part in (ip, location, org) if part)
            self.log(f"{label}检测成功，出口信息: {summary}")
            if require_japan and country != "JP":
                raise RuntimeError(f"{label}出口不是日本，已停止: {summary or text.strip()[:120]}")
        except Exception as exc:
            if require_japan:
                raise RuntimeError(f"{label}检测失败或非日本: {exc}")
    ...
    def _register(self, page, context) -> None:
        ...
        while time.time() < deadline:
            ...
            if "add-phone" in url or "phone-verification" in url:
                if self.registration_flow == "phone":
                    self._handle_browser_phone_verification(page)
                    email_code_submitted = False
                    about_you_submitted = False
                    continue
                raise RuntimeError("当前账号触发手机验证，注册流程选择的是现有邮箱验证码，已停止")
            if "phone-otp/select-channel" in url:
                if self.registration_flow == "phone":
                    self._send_existing_phone_otp_from_browser(page)
                    email_code_submitted = False
                    about_you_submitted = False
                    continue
                raise RuntimeError("当前账号触发手机验证码通道选择，注册流程选择的是现有邮箱验证码，已停止")
    ...
    def _handle_browser_phone_verification(self, page) -> None:
        if not self.phone_provider:
            raise RuntimeError("注册流程选择手机号注册，但未配置手机号池")
        last_error = ""
        while True:
            phone = self.phone_provider("next", self.account.email, "")
            if not phone:
                raise RuntimeError(f"手机号池没有可用手机号" + (f": {last_error}" if last_error else ""))
            phone_number = str(phone.get("number") or "").strip()
            self.log(f"注册流程提交手机号: {phone_number}")
            try:
                page.goto(f"{AUTH_BASE_URL}/add-phone", wait_until="domcontentloaded", timeout=90000)
                continue_url = self._send_phone_otp_from_browser(page, phone_number)
                if continue_url:
                    page.goto(continue_url, wait_until="domcontentloaded", timeout=90000)
                code = self.phone_provider("code", self.account.email, phone)
                if not code:
                    raise RuntimeError("短信链接未读取到验证码")
                self.log(f"读取到注册手机号短信验证码: {code}")
                continue_url = self._validate_phone_otp_from_browser(page, str(code))
                if continue_url:
                    page.goto(continue_url, wait_until="domcontentloaded", timeout=90000)
                return
            except Exception as exc:
                last_error = str(exc)
                self.phone_provider("bad", self.account.email, {**phone, "error": last_error})
                self.log(f"注册手机号 {phone_number} 不可用，切换下一个: {last_error}")

    def _send_phone_otp_from_browser(self, page, phone_number: str) -> str:
        result = page.evaluate(
            ...
            {"url": AUTH_PHONE_SEND_URL, "phoneNumber": phone_number},
        )
        if not result.get("ok"):
            raise RuntimeError(f"SendPhoneOtp请求失败: {str(result.get('text') or result.get('status'))[:500]}")
        payload = result.get("data") or {}
        return normalize_auth_continue_url(str(payload.get("continue_url") or ""))

    def _send_existing_phone_otp_from_browser(self, page) -> None:
        self.log("注册流程遇到手机验证码通道选择，自动选择短信")
        result = page.evaluate(
            """async ({url}) => {
                const resp = await fetch(url, {
                    method: 'POST',
                    ...
                    headers: {
                        ...
                        'content-type': 'application/json',
                        origin: 'https://auth.openai.com',
                        referer: 'https://auth.openai.com/phone-otp/select-channel',
                    },
            ...
            {"url": AUTH_PHONE_OTP_SEND_URL},
        )
        if not result.get("ok"):
            raise RuntimeError(f"PhoneOtpSend请求失败: {str(result.get('text') or result.get('status'))[:500]}")
        payload = result.get("data") or {}
        continue_url = normalize_auth_continue_url(str(payload.get("continue_url") or f"{AUTH_BASE_URL}/phone-verification"))
        page.goto(continue_url, wait_until="domcontentloaded", timeout=90000)

    def _validate_phone_otp_from_browser(self, page, code: str) -> str:
        result = page.evaluate(
            """async ({url, code}) => {
                const resp = await fetch(url, {
                    method: 'POST',
            ...
            {"url": AUTH_PHONE_OTP_VALIDATE_URL, "code": code},
        )
        if not result.get("ok"):
            raise RuntimeError(f"PhoneOtpValidate请求失败: {str(result.get('text') or result.get('status'))[:500]}")
        payload = result.get("data") or {}
        return normalize_auth_continue_url(str(payload.get("continue_url") or ""))
    ...
    def _extract_pay_link(self, page) -> dict:
        ...
        link_label = "试用短链" if trial_short_link else "支付长链接"
            ...
            try:
                ...
                result = page.evaluate(
                    """async ({country, currency}) => {
                        ...
                        if (!resp.ok || !url) throw new Error(`生成支付链接失败: HTTP ${resp.status} ${text.slice(0, 500)}`);
                        return {
                            url,
                            checkoutUrl: url,
                ...
                if result and result.get("url"):
                    ...
                    if trial_short_link:
                        self.log("试用短链已生成；打开时请使用美国支付代理以触发 PayPal")
                    else:
                        self.log("支付长链接已生成，注册浏览器窗口保持打开")
                    return {
                        "url": str(result.get("url") or ""),
                        "checkout_url": str(result.get("checkoutUrl") or ""),
                        "access_token": str(result.get("accessToken") or ""),
                        "session_json": json.dumps(result.get("session") or {}, ensure_ascii=False, indent=2),
                    }
            except Exception as exc:
                last_error = exc
...
class App:
    def __init__(self, root: Tk):
        ...
        self.payment_mode = StringVar(value="PayPal 长链接 JP/JPY")
        self.registration_flow = StringVar(value="现有邮箱验证码注册")
        ...
        self.require_japan_extract_proxy = BooleanVar(value=True)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(proxy_frame, text="提取长链强制日本出口（勾选=非 JP 代理直接失败并换代理）", variable=self.require_japan_extract_proxy).pack(anchor="w", pady=(6, 0))
        ...
        ttk.Label(row2, text="注册流程").pack(side=LEFT, padx=(8, 0))
        ttk.Combobox(row2, textvariable=self.registration_flow, values=list(REGISTRATION_FLOWS.keys()), state="readonly", width=18).pack(side=LEFT, padx=8)
        ttk.Checkbutton(row2, text="无头浏览器", variable=self.headless).pack(side=LEFT, padx=8)
        ttk.Button(row2, text="注册并获取Session信息", command=self.start_selected).pack(side=LEFT, padx=(8, 8))
    ...
    def load_state(self) -> None:
        ...
        try:
            ...
            saved_registration_flow = str(settings.get("registration_flow") or "")
            if saved_registration_flow in REGISTRATION_FLOWS:
                self.registration_flow.set(saved_registration_flow)
    ...
    def save_state(self) -> None:
        data = {
            ...
            "settings": {
                ...
                "registration_flow": self.registration_flow.get(),
    ...
    def _start_worker(self, accounts: list[MailAccount]) -> None:
        ...
        self.save_state()
        mode = self.payment_mode.get()
        registration_flow = REGISTRATION_FLOWS.get(self.registration_flow.get(), "email")
        headless = bool(self.headless.get())
        local_proxy = normalize_proxy_url(self.local_proxy.get())
        use_payment_proxy_for_register = bool(self.register_with_payment_proxy.get())
        dynamic_proxies = [] if use_payment_proxy_for_register else self._take_dynamic_proxies(len(accounts))
        payment_dynamic_proxy = self._peek_payment_dynamic_proxy()
        threading.Thread(target=self._run_accounts, args=(accounts, mode, registration_flow, headless, local_proxy, dynamic_proxies, payment_dynamic_proxy, use_payment_proxy_for_register), daemon=True).start()
    ...
    def _run_accounts(self, accounts: list[MailAccount], mode: str, registration_flow: str, headless: bool, local_proxy: str, dynamic_proxies: list[str], payment_dynamic_proxy: str, use_payment_proxy_for_register: bool) -> None:
        try:
            ...
            for start in range(0, len(accounts), concurrency):
                ...
                for account in accounts[start:start + concurrency]:
                    ...
                    thread = threading.Thread(
                        ...
                        args=(account, mode, registration_flow, headless, local_proxy, register_dynamic_proxy, extract_dynamic_proxy, use_payment_proxy_for_register),
                        daemon=True,
                    )
                    thread.start()
    ...
    def _run_account_thread(self, account: MailAccount, mode: str, registration_flow: str, headless: bool, local_proxy: str, register_dynamic_proxy: str, extract_dynamic_proxy: str, use_payment_proxy_for_register: bool) -> None:
        if self.stop_event.is_set():
            return
        ...
        try:
            with ProxyChainServer(local_proxy, register_dynamic_proxy, lambda msg: self.events.put(("log", msg))) as register_chain:
                ...
                worker = OpenAIRegisterPayLinkWorker(account, mode, headless, register_proxy, extract_proxy, lambda msg: self.events.put(("log", msg)), registration_flow, self._phone_provider)
                result = worker.run()
            self.events.put(("account-updated", account.email))
    ...
    def _payment_profile_dir(self, email_addr: str) -> Path:
        key = re.sub(r"[^A-Za-z0-9_.@-]+", "_", (email_addr or "shared-payment-window").strip())[:120]
        if not key:
            key = "shared-payment-window"
        return PAYMENT_PROFILE_DIR / key

    def _payment_profile_fingerprint(self, profile_dir: Path) -> DeviceFingerprint:
        path = profile_dir / "opencode-fingerprint.json"
        if path.exists():
            try:
                return fingerprint_from_dict(json.loads(path.read_text(encoding="utf-8")))
            except Exception as exc:
                self.events.put(("log", f"支付窗口指纹读取失败，重新生成: {exc}"))
        fingerprint = generate_fingerprint()
        path.write_text(json.dumps(fingerprint_to_dict(fingerprint), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        return fingerprint
    ...
    def _open_payment_link_worker(self, link: str, local_proxy: str, dynamic_proxy: str, extension_dir: str, paypal_phone: str, paypal_card: str, paypal_sms_url: str, email_addr: str = "") -> None:
        ...
        try:
            ...
            with ProxyChainServer(local_proxy, dynamic_proxy, lambda msg: self.events.put(("log", msg))) as chain:
                ...
                profile_path = self._payment_profile_dir(email_addr)
                profile_path.mkdir(parents=True, exist_ok=True)
                fingerprint = self._payment_profile_fingerprint(profile_path)
                profile_dir = str(profile_path)
                self._seed_payment_browser_preferences(profile_dir)
                self.events.put(("log", f"支付窗口复用浏览器环境: {profile_dir}"))
                args = [
                    "--disable-blink-features=AutomationControlled",
                        ...
                        for current_page in list(context.pages):
                            ...
                            if is_paypal_signup_page:
                                ...
                                if key not in fill_ready_at:
                                    ...
                                    self.events.put(("log", f"检测到 PayPal 创建账户页，等待 10 秒后自动填写: {current_page.url[:80]}"))
    ...
    def _generate_opll_link_retry_worker(self, account: MailAccount, access_token: str, local_proxy: str, proxy_queue: queue.Queue, index: int, total: int, reuse_proxy_enabled: bool = False) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            if self.stop_event.is_set():
                break
            self.events.put(("log", f"[{account.email}] 长链提取失败，已移除当前支付代理，换下一个代理继续重试"))
            time.sleep(1)
        self.events.put(("log", f"[{account.email}] 批量提取长链已停止"))
    ...
    def _generate_opll_link_for_account(self, account: MailAccount, access_token: str, local_proxy: str, payment_dynamic_proxy: str) -> bool:
        try:
            ...
            self.events.put(("status", account.email, "提取PP链中"))
            self.events.put(("log", f"[{account.email}] 按截图逻辑提取 PayPal approve 长链，代理: {proxy.label}"))
            with ProxyChainServer(local_proxy, payment_dynamic_proxy, lambda msg: self.events.put(("log", msg))) as chain:
                ...
                if not long_url:
                    raise RuntimeError(f"接口提取成功但没有返回长链: {result}")
                is_fallback = bool(result.get("fallback"))
                self.events.put(("result", account.email, {"url": long_url, "checkout_url": long_url, "access_token": access_token, "link_proxy": used_proxy, "link_proxy_label": proxy.label, "link_proxy_exit": proxy_exit}))
                if is_fallback:
                    self.events.put(("status", account.email, "已回退hosted"))
                    self.events.put(("log", f"[{account.email}] PP链提取失败，已按源码逻辑回退 hosted 长链: {result.get('provider_error') or ''}"))
                else:
                    self.events.put(("status", account.email, "长链已提取"))
                    self.events.put(("log", f"[{account.email}] PayPal approve 长链提取完成: {long_url}"))
                return True
        except Exception as exc:
