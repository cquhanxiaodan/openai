...
class App:
    ...
    def generate_link_from_selected_session(self) -> None:
        ...
        storage_state_text = str(payload.get("storage_state_json") or "").strip()
        if not storage_state_text:
            messagebox.showwarning(APP_TITLE, "当前邮箱暂无浏览器登录态，请先重新执行“注册并获取Session信息”")
            return
        if self.running:
            messagebox.showinfo(APP_TITLE, "任务正在运行")
            return
        ...
        threading.Thread(target=self._open_trial_payment_from_session_worker, args=(account, storage_state_text, local_proxy, payment_dynamic_proxy), daemon=True).start()
