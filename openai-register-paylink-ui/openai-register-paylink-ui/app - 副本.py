...
def opll_stripe_init(cs_id: str, country: str, currency: str, proxy_url: str = "", payment_locale: str = "en") -> dict:
...
def opll_stripe_context(init_payload: dict, payment_locale: str = "en") -> dict:
...
def opll_stripe_payment_page_redirect_url(stripe: requests.Session, cs_id: str, payment_locale: str = "en", timeout_seconds: int = 45) -> str:
...
def generate_opll_paypal_long_link(access_token: str, country: str, currency: str, proxy_url: str = "") -> dict:
    ...
    return_url = opll_to_openai_pay_url(stripe_hosted_url) or opll_stripe_checkout_long_url(checkout["cs_id"], checkout["billing_country"], checkout.get("processor_entity", ""))
    confirm = stripe.post(
        ...
        data={
            ...
            "version": "5c62bd78d8",
            "expected_amount": ctx["checkout_amount"],
    ...
    if not stripe_redirect_url and isinstance(submission, dict) and submission.get("state") == "requires_approval":
        ...
        stripe_redirect_url = opll_stripe_payment_page_redirect_url(stripe, checkout["cs_id"])
    if not stripe_redirect_url:
        stripe_redirect_url = opll_stripe_payment_page_redirect_url(stripe, checkout["cs_id"], timeout_seconds=30)
    provider_url = opll_resolve_external_redirect(stripe, stripe_redirect_url)
    return {**checkout, "payment_method_id": pm_id, "stripe_hosted_url": stripe_hosted_url, "stripe_redirect_url": stripe_redirect_url, "provider_redirect_url": provider_url, "long_url": provider_url or stripe_redirect_url}
...
class App:
    ...
    def generate_link_from_selected_session(self) -> None:
        ...
        self.save_state()
        local_proxy = normalize_proxy_url(self.local_proxy.get())
        payment_dynamic_proxy = self._take_payment_dynamic_proxy()
    ...
    def _generate_opll_link_from_session_worker(self, account: MailAccount, access_token: str, local_proxy: str, payment_dynamic_proxy: str) -> None:
        try:
            ...
            with ProxyChainServer(local_proxy, payment_dynamic_proxy, lambda msg: self.events.put(("log", msg))) as chain:
                ...
                if "paypal.com/agreements/approve" not in long_url:
                    raise RuntimeError(f"未提取到 PayPal approve 长链，返回: {long_url}")
                self.events.put(("result", account.email, {"url": long_url, "checkout_url": long_url, "access_token": access_token}))
